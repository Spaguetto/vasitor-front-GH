...
const RolesGroupedList = ({arr, deps, deleteModalHandler, editModalHandler, editModalData}) => {
    // TODO: Ajustar filtros de lista de roles
    // const {search, category} = deps ? deps : {search: '', category: ''};
    return <List>{arr.map((item, i) => <Item key={`${item.id}-${item.roleName}-${item.isActive}`} data={item} deleteModalHandler={deleteModalHandler} editModalHandler={editModalHandler} editModalData={editModalData}/>)}</List>
}
